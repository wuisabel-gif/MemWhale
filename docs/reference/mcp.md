...
| `search_memory` | Search remembered commands, sessions, and notes for a term, ranked by the explainable engine. | `query` (string, **required**); `project`, `machine` (string, optional scope); `agent` (optional: `claude`, `rho`, `cursor`, or `terminal`) | Ranked hits with a score, a snippet, producing-agent label, and the reasons each ranked where it did. |
| `get_context` | The most relevant remembered memory, engine-ranked, optionally scoped. | `project`, `machine` (string, optional); `agent` (optional: `claude`, `rho`, `cursor`, or `terminal`) | Up to 8 ranked hits, each with score, snippet, producing-agent label, and reasons. |
