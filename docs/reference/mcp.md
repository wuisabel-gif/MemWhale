...
| `search_memory` | Search remembered commands, sessions, and notes for a term, ranked by the explainable engine. | `query` (string, **required**); `project`, `machine` (string, optional scope); `agent` (optional: `claude`, `rho`, `cursor`, `codewhale`, or `terminal`) | Ranked hits with a score, a snippet, producing-agent label, and the reasons each ranked where it did. |
