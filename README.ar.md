...
**MemoryWhale 0.11.0 — Explainable Debugging Evidence · 20 سبتمبر 2026.**
تشترك واجهة سطر الأوامر وواجهة الويب وتطبيق سطح المكتب في إصدار المنتج 0.11.0،
...
للاطلاع على دليل الترقية. ويبقى مخطط قاعدة البيانات على الإصدار 10.
...
- **ذاكرة واحدة لجميع وكلاء البرمجة.** يستطيع أي عميل متوافق مع MCP عبر
...
- **اجعل مصدر الأدلة صريحاً.** يخزّن مخطط قاعدة البيانات 10 الوكيلَ الذي نفّذ
  الأمر بإحدى القيم `claude` أو `rho` أو `NULL`. ويدل وسم العرض والتصفية
  `terminal` على مصدر طرفي أو يدوي أو قديم، ولا يثبت أن إنساناً نفّذ الأمر. وهوية الوكيل
  مستقلة عن نوع المصدر، مثل `command` أو `session` أو `note`.
- **مستودع واحد مع تمييز أشجار العمل.** تجمع معرّفات المستودعات
...
- **استرجع سياق GitHub عند الطلب.** يقرأ `mw github context <pr>` بيانات
  ...
  سياقاً محدود الحجم ومنقّحاً من البيانات الحساسة، دون سحب الشيفرة (checkout)
  أو حفظ السياق تلقائياً في الذاكرة. لا توجد مزامنة GitHub
  تعمل في الخلفية.
...
- تعالج أخطاء البناء أو الاعتماديات أو Git أو البيئات أو النشر؛
