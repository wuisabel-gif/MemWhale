...
pub fn handle_http_rpc(body: &str) -> HttpMcpReply {
    ...
    let mut unused_legacy_name = None;
    let mut unused_legacy_initialized = false;
    match rpc_reply(
        &msg,
        Transport::Http,
        &mut unused_legacy_initialized,
        &mut unused_legacy_name,
    ) {
        None => HttpMcpReply {
            status: "202 Accepted",
            body: String::new(),
...
fn dispatch(
    ...
    if transport == Transport::Http {
        return Err(RpcError::new(-32602, "HTTP MCP requires 2026-07-28 _meta"));
    }
