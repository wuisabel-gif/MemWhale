...
/// Reject symlinked directory components so an export cannot follow a redirected parent.
fn reject_symlink_parents(path: &Path) -> Result<(), String> {
    ...
    for component in path.components() {
        match component {
            ...
            Component::Normal(name) => {
                ...
                {
                    return Err(format!(
                        "refusing symlinked output path component {}",
                        current.display()
                    ));
                }
            }
...
pub fn export(
    ...
    for key in ids {
        ...
        if kind == "command" {
            ...
            evidence.push(Record{id:key.clone(),kind:kind.into(),evidence:ev,provenance:serde_json::json!({"cwd":r.2.as_deref().map(crate::redact),"created_at":r.6,"agent":r.7}),source_ids:vec![key.clone()],argv_json:Some(argv_json),invocation:Some(invocation),exit_code:r.3,exit_outcome:Some(outcome(r.3))});
        } else {
            let r = conn
                .query_row(
            ...
            evidence.push(Record{id:key.clone(),kind:kind.into(),evidence:bounded(r.0,&mut notices,"transcript"),provenance:serde_json::json!({"cwd":r.1.as_deref().map(crate::redact),"started_at":r.2,"ended_at":r.3,"status":r.4}),source_ids:vec![key.clone()],argv_json:None,invocation:None,exit_code:None,exit_outcome:None});
        }
    }
    let h = Handoff {
        format_version: 1,
