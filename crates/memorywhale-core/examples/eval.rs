...
impl EmbedStats {
    fn mean_cold_ms(&self) -> f64 {
        if self.cold_calls == 0 {
            ...
            self.total_cold_ms as f64 / self.cold_calls as f64
        }
    }
}
...
fn main() -> anyhow::Result<()> {
    ...
    let semantic_available = match embed_all(&embedder, &mem_texts, &mut cache, &mut stats) {
        Ok(map) => {
            ...
            for m in memories.iter_mut() {
                if let Some(v) = map.get(&m.text) {
                    m.embedding = Some(v.clone());
                }
            }
            for q in &ds.queries {
                if let Some(v) = map.get(&q.text) {
                    query_embeddings.insert(q.text.clone(), v.clone());
                }
            }
    ...
    println!(
        "{:<20} {:>6} {:>6} {:>6} {:>6} {:>10}",
        "config", "P@5", "R@5", "MRR", "nDCG5", "score_us/q"
    );
    for cfg in &configs {
        ...
        for q in &ds.queries {
            ...
            let ranked: Vec<bool> = scored
                .iter()
                .map(|s| relset.contains(&s.memory.id))
                .collect();
